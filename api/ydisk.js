 ...
 */
// Две формы ссылки, и обе нужны: `releases/download/<метка>/<файл>` — на конкретный выпуск,
// `releases/latest/download/<файл>` — постоянная «последняя версия» (её использует страница
// раздачи и проверка обновлений). Первая проверка пропускала только первую форму, и постоянная
// ссылка получала отказ — поймано замером 28.08.2026, а не рассуждением.
const ALLOWED_RELEASE = /^https:\/\/github\.com\/Weaverden\/gokifu\/releases\/(latest\/)?download\//i;
